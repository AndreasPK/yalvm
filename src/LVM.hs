...
tailCall state = do
  ...
  if not luaCall then runHaskellCall state else do

    let lof@(LOFunction calledFunction) = callee

    --TODO:Call haskell function?
    ...
    let (fixedArgs, varArgs) = if luaCall then splitAt maxArgCount parameters else (parameters, [])
